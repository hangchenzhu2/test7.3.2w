...
class AlertManager {
    ...
    async updateAlerts(lat, lon) {
        try {
            ...
            // 去重处理：根据title和severity去除重复的alert
            this.activeAlerts = this.deduplicateAlerts(alerts);
            this.displayAlerts();
    ...
    deduplicateAlerts(alerts) {
        ...
        return alerts.filter(alert => {
            const key = `${alert.title}-${alert.severity}-${alert.description}`;
            if (seen.has(key)) {
                return false;
            }
            seen.add(key);
    ...
    createAlertHTML(alert) {
        ...
        // 格式化描述文字：遇到*号时换行
        const formattedDescription = this.formatAlertDescription(alert.description);
    ...
    // 格式化预警描述文字
    formatAlertDescription(description) {
        // 在每个*号前添加换行，但跳过开头的*
        return description
            .replace(/\s+\*/g, '<br/>*') // 在*号前添加换行
            .replace(/^\*/g, '*') // 确保开头的*号不会有多余换行
            .trim();
    }
    ...
    showBanner(alert) {
        ...
        if (alertTitle && alertDescription) {
            ...
            // 格式化横幅描述文字
            alertDescription.innerHTML = this.formatAlertDescription(alert.description);
        }
